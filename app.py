...
import base64
import time
from src.crew import NutriBotRecipeCrew, NutriBotAnalysisCrew

def format_recipe_output(final_output):
    """
    Formats the recipe output into a table-based Markdown format.
    
    :param final_output: The output from the NutriBotRecipe workflow.
...
def format_analysis_output(final_output):
    ...
    :param final_output: The JSON output from the NutriBotAnalysis workflow.
    :return: Formatted output as a Markdown string.
    """
...
def analyze_food(image, dietary_restrictions, workflow_type, progress=gr.Progress(track_tqdm=True)):
    ...
    :return: Result from the NutriBot workflow.
    """
    ...
    if workflow_type == "recipe":
        crew_instance = NutriBotRecipeCrew(
            image_data=image_path,
            dietary_restrictions=dietary_restrictions
        )
    elif workflow_type == "analysis":
        crew_instance = NutriBotAnalysisCrew(
            image_data=image_path
        )
    else:
...
function createGradioAnimation() {
    ...
    var text = 'Welcome to your AI NutriBot!';
